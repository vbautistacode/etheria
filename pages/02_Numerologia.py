...
def _sync_sidebar_dob_to_tab():
    st.session_state["num_dob"] = st.session_state.get("dob", st.session_state.get("num_dob", date(2026,1,1)))
...
with tab_influencias:
    ...
    # --- criar coluna de exibição com rótulos em pt_BR (mantendo valor original para lógica) ---
    def _planet_label_from_cell(x):
        try:
            if pd.isna(x) or not x:
                return "—"
            # tenta normalizar via influences (canonical -> label pt_BR)
            if influences:
                canonical = influences.to_canonical(x) or x
                label = influences.planet_label_pt(canonical) or str(x)
                return label
            # fallback simples: retorna o valor original
            return str(x)
        except Exception:
            return str(x)
    ...
    st.success("**Roadmap de ciclos (ordem de interpretação)**")

    # garantir result como fallback
    result = locals().get('result') or {}

    for idx, row in df_cycles_display.reset_index(drop=True).iterrows():
        # raw value (pode ser canônico ou label)
        raw_planet = row["planet"] if pd.notna(row["planet"]) else None

        # canonical para lógica/lookup
        try:
            canonical_planet = influences.to_canonical(raw_planet) if raw_planet and influences else (raw_planet or None)
        except Exception:
            canonical_planet = raw_planet or None

        # label em pt_BR para exibição
        planet_label = row.get("planet_label") or (influences.planet_label_pt(canonical_planet) if canonical_planet and influences else (raw_planet or "—"))
        ...
        # tentar obter interpretação específica do módulo influences (usar canonical_planet)
        interp_short = None
        interp_long = None
        try:
            ...
            elif canonical_planet and hasattr(influences, "interpret_planet"):
                p = influences.interpret_planet(canonical_planet, birth_year=birth_year, age=birth_age)
                if isinstance(p, dict):
                    interp_short = p.get("short")
                    interp_long = p.get("long")
            elif hasattr(influences, "interpret_combined"):
                try_sources = {"year": canonical_planet or raw_planet, "hour": canonical_planet or raw_planet, "weekday": canonical_planet or raw_planet}
        ...
        if not interp_short and not interp_long:
            try:
                if isinstance(result, dict) and result.get("by_planet"):
                    # tentar canonical primeiro, depois label (compatibilidade)
                    p = None
                    if canonical_planet:
                        p = result["by_planet"].get(canonical_planet)
                    if not p:
                        p = result["by_planet"].get(planet_label)
                    if p:
                        interp_short = p.get("short")
                        interp_long = p.get("long")
                if not interp_short:
        ...
        with st.expander(header, expanded=is_current):
            ...
            if is_current:
                st.markdown(
                    "<div style='padding:6px;border-left:6px solid #006100;background:#C6EFCE textcolor:#C6EFCE', gap:1px, margin:2px>"
